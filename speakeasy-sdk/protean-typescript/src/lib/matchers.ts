...
import { ProteanDefaultError } from "../models/errors/protean-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ProteanDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ProteanDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ProteanDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ProteanDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
