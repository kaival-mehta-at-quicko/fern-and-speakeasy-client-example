...
async function run() {
  const result = await protean.bank.verifyBankAccount({
...
```typescript
import { ProteanCore } from "proteanclient/core.js";
import { kycVerifyBankAccount } from "proteanclient/funcs/kyc-verify-bank-account.js";

// Use `ProteanCore` for best tree-shaking performance.
// You can create one instance of it to use across an application.
const protean = new ProteanCore({
  serverURL: "https://api.example.com",
  bearer: process.env["PROTEAN_BEARER"] ?? "",
});

async function run() {
  const res = await kycVerifyBankAccount(protean, {
    apikey: "kidNaidREXqRUp9qPsrJcLC1pbYJ7Y0q97dFw3gHTf2gbXQF",
    body: {
      entityId: "78bea7b4-1caf-4528-96e4-ec7f9f43f0db",
      programId: "310",
  ...
  if (res.ok) {
    ...
    console.log("kycVerifyBankAccount failed:", res.error);
  }
}
